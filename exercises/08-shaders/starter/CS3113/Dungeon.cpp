...
void Dungeon::initialise()
{
   ...
   */
   mGameState.camera = { 0 };
   mGameState.camera.target = mGameState.hinako->getPosition();
...
void Dungeon::update(float deltaTime)
{
   ...
   Vector2 currentPlayerPosition = { mGameState.hinako->getPosition().x, mOrigin.y };

   if (mGameState.hinako->getPosition().x > mGameState.map->getLeftBoundary() + (GetScreenWidth() / 2) &&
       mGameState.hinako->getPosition().x < mGameState.map->getRightBoundary() - (GetScreenWidth() / 2))
